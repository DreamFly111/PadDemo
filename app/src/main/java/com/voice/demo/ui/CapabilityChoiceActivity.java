...
import com.voice.demo.group.BaseChatActivity;
 ...
 * 
 * <p>
 * Title: CapacityChoiceActivity.java
 * </p>
 * <p>
 * Description:
 * </p>
...
public class CapabilityChoiceActivity extends CCPBaseActivity implements CCPCapabilityItemView.OnCapacityItemClickListener, CCPAlertDialog.OnPopuItemClickListener, OnClickListener {
	 ...
	 *            The Capacity View title
	 * @param iconResid
	 *            The capacity view icon
	 * @param bgResid
	 *            The capacity view background resource.
	 * @return
	 */
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		/*
		 * // VOIP免费电话 if (!phoneStr.startsWith("8") || phoneStr.length() != 14) { // 判断输入合法性 CCPApplication.getInstance().showToast( getString(R.string.voip_number_format)); return; }
		 */
	...
	private void startVoIPCallAction() {
		if (!TextUtils.isEmpty(phoneStr)) {
			...
			intent.putExtra(BaseChatActivity.KEY_GROUP_ID, phoneStr);
	...
	private void checkSDKversion() {
		...
		if (sdkVersion != null) {
			String tips = getString(R.string.str_sdk_support_tips, Build.SDK_VERSION_NAME, String.valueOf(Build.LIBVERSION.AUDIO_SWITCH), String.valueOf(Build.LIBVERSION.VIDEO_SWITCH), CCPApplication.getInstance().getVersion());
	 ...
	 */
	@Override
	public void onItemClick(ListView parent, View view, int position, int resourceId) {
		switch (resourceId) {
